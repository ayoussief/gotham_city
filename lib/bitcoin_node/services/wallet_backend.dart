...
class WalletBackend {
  ...
  Future<String> importPrivateKey(String privateKey) async {
    // For single private key import (non-HD)
  ...
  Future<String> sendTransaction(String toAddress, double amount, double feeRate) async {
    ...
    final txHex = await buildTransaction(toAddress, amount, feeRate);
  ...
  String _deriveAddressFromPrivateKey(String privateKey) {
    // Simplified address derivation from private key
    // In a real implementation, this would use proper Bitcoin address derivation
    final hash = sha256.convert(utf8.encode(privateKey));
    return 'gt1q${hash.toString().substring(0, 32)}';
  }
